...
/// struct for typed errors of method [`handle_service_status_request`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum HandleServiceStatusRequestError {
...
pub async fn handle_service_status_request(configuration: &configuration::Configuration, organization: &str, cluster: &str, project: Option<&str>, environment: Option<&str>) -> Result<models::ServiceStatusDto, Error<HandleServiceStatusRequestError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ServiceStatusDto`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ServiceStatusDto`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<HandleServiceStatusRequestError> = serde_json::from_str(&content).ok();
